...
class UserRepository
{
    ...
    {
        $user = new User;
        // updated by Shailesh to remove username from register form
        $EmailChars = ['@','.','_','-'];
        $user->username = str_replace($EmailChars,'',array_get($input, 'email'));
        $user->email    = array_get($input, 'email');
        $user->password = array_get($input, 'password');

        // updated by Shailesh to allow additional fields in register form
        $user->first_name = array_get($input, 'first_name');
        $user->last_name = array_get($input, 'last_name');
        $user->store__name = array_get($input, 'store__name');
